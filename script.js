...
function main() {
  ...
  let render = () => {
    ...
    ctx.fillStyle = 'white'
    ctx.fillText("FPS:" + String(fps), 16, 32)
    ctx.fillStyle = 'white'
  ...
  let safe_render = () => {
    ...
    let delay = durationTarget - (timeNow - timeStart)
    // ラグが大きいとき遅滞を抑える
    if (delay < -MSPF) durationTarget -= (delay + MSPF)
    setTimeout(safe_render, Math.max(0, delay))
  }
  safe_render()
}
